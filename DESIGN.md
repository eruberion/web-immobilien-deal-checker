...
- Invalid/fehlend: sichtbare Feldmeldung plus Fokus, keine stille NaN-/Nullrechnung.
...
- Info-Button ist per Tastatur erreichbar, hat einen zugänglichen Namen und zeigt Hilfe auf Hover sowie Fokus.
- Tooltip enthaelt Erklaerung, aber keine alleinige rechtliche oder sicherheitskritische Information.
- Auf Touchgeraeten braucht Hilfe einen stabilen Tap-/Dismiss-Pfad.

### Score und Deal-Meter

- Initialzustand lautet klar `Noch keine Berechnung`.
...
- Ziel: WCAG 2.2 AA.
- Vollstaendiger sichtbarer `:focus-visible`-Stil fuer Buttons, Inputs, Selects und Tooltip-Trigger; Fokusreihenfolge folgt Abschnittsnummern, dann Ergebnis.
- Labels, Beschreibungen und Fehlermeldungen sind programmatisch zugeordnet.
...
- `prefers-reduced-motion` deaktiviert Button-Transform, Tooltip-Fade und sonstige nicht notwendige Uebergaenge.
- Zoom bis 200 % und 320-px-Breite duerfen keine Eingabe, Scoreerklaerung oder Advice Boundary verlieren.
- Dynamisch aktualisierte Ergebniszusammenfassung braucht eine angemessene Live-Region, ohne jede KPI einzeln vorzulesen.
